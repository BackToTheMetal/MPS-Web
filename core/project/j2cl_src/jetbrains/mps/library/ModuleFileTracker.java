...
public final class ModuleFileTracker {
  private final Map<IFile, Set<SModuleReference>> myFile2Module = new HashMap<>();
  ...
  public void track(@NotNull IFile file, @NotNull SModuleReference moduleRef) {
    Set<SModuleReference> modules = myFile2Module.computeIfAbsent(file, k -> new HashSet<>());
  ...
  public void forget(@NotNull IFile file) {
    final Set<IFile> files2Remove = new HashSet<>();
  ...
  public Map<SModuleReference, IFile> getAffectedBy(Collection<IFile> files) {
    ...
    HashMap<SModuleReference, IFile> rv = new HashMap<>();
    for (IFile moduleFile : myFile2Module.keySet()) {
...
//    files.stream().flatMap(f -> myFile2Module.getOrDefault(f, Collections.emptySet()).stream()).collect(Collectors.toSet());
    final HashMap<SModuleReference, IFile> rv = new HashMap<>();
    for (IFile f : files) {
  ...
  public static class Delta {
    ...
    private Delta(Map<SModuleReference, IFile> trackedToChangeCandidates, HashMap<SModuleReference, ModuleHandle> newlyDiscovered,
