...
public class SRepositoryEventsDispatcher {
  private final List<SRepositoryListener> myListeners = new ArrayList<>();
  ...
  private final HashMap<SRepositoryListener, Integer> myListenerCount = new HashMap<>();
  ...
  public final void addRepositoryListener(SRepositoryListener listener) {
    myListenerCount.putIfAbsent(listener, new Integer(0));
    if (myListenerCount.get(listener) + 1 != 1) {
  ...
  public final void removeRepositoryListener(SRepositoryListener listener) {
    Integer lc = myListenerCount.get(listener);
    if (lc == null || lc -1 != 0) {
