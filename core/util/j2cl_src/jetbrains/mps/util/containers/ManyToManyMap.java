...
public class ManyToManyMap<F, S> {
  private Map<F, Set<S>> myFToS = new HashMap<>();
  private Map<S, Set<F>> mySToF = new HashMap<>();
  ...
  public void clearSecond(S s) {
    ...
    for (F f : new HashSet<>(mySToF.get(s))) {
      removeLink(f, s);
    }
  }
  ...
  public void addLink(F f, S s) {
    if (!myFToS.containsKey(f)) {
      myFToS.put(f, new HashSet<>(1));
    }
    if (!mySToF.containsKey(s)) {
      mySToF.put(s, new HashSet<>(1));
    }
    myFToS.get(f).add(s);
    mySToF.get(s).add(f);
  }
