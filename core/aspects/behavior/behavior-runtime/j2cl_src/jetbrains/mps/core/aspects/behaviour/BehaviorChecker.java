...
final class BehaviorChecker {
  ...
  static <T> void checkParameters(@NotNull BHDescriptor originalDescriptor, @NotNull SMethod<T> method, @NotNull Object[] arguments) throws BHMethodArgumentsCountDoNotMatch, BHArgumentsDoNotMatch{
    ...
    final boolean hasVarArg = !declaredParameters.isEmpty() && declaredParameters.get(declaredParameters.size() - 1) instanceof SVarArgParameter;
    if (!hasVarArg) {
      if (declaredParameters.size() != arguments.length) {
        String m = "Method " + method +
                   " expects " + declaredParameters.size() +
                   " parameters while received " + arguments.length;
      ...
      if (arguments[i] != null) {
        ...
        if (hasVarArg && (i >= declaredParameters.size() - 1)) { // that lies in a vararg argument.
          ...
          if (!varArgType.getComponentType().isAssignableFrom(passedObjectType)) {
            throw new BHArgumentsDoNotMatch(
                "Argument does not match vararg parameter " +
                declaredParameters.get(declaredParameters.size() - 1) +
                ": received " + arguments[i] +
                " for method " + method +
                " (" + originalDescriptor + ")"
            );
          }
          ...
          if (!param.getType().isAssignableFrom(passedObjectType)) {
            throw new BHArgumentsDoNotMatch(
                "Argument " + declaredParameters.get(declaredParameters.size() - 1) +
                " does not match value " + arguments[i] +
                " for method " + method +
                " (" + originalDescriptor + ")"
            );
          }
  ...
  private static void checkConceptIsValid(@NotNull SAbstractConcept someConcept) {
    if (!someConcept.isValid()) {
      throw new ConceptIsNotValidException(
          "The concept " + someConcept +
          " is not valid, probably the language " +
