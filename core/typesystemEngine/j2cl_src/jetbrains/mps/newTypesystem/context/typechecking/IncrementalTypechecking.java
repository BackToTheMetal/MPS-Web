...
/**
 * J2CL variant: {@code gnu.trove.THashMap}/{@code THashSet} are not available in the J2CL
 * build (no J2CL-compatible Trove target), so this variant uses plain {@code HashMap}/
 * {@code HashSet}.
 * <p>
 * Additionally, {@code java.lang.ref.WeakReference}/{@code ReferenceQueue} and
 * {@code jetbrains.mps.util.WeakSet} are not available in the J2CL JRE emulation. This
 * variant's {@code MyModelListenerManager} therefore tracks nodes with plain (strong)
 * references instead of weak ones: {@link MyModelListenerManager#updateGCedNodes()} becomes
 * a no-op, since GC of tracked nodes can no longer be detected. This means a model listener
 * attached for a root that becomes unreachable is not proactively detached in the J2CL/browser
 * runtime; it is expected to be cleaned up via {@link #dispose()} instead.
 */
public class IncrementalTypechecking extends ReportingTypechecking<State, TypeSystemComponent> {
  ...
  private final Map<SModel, Set<SNode>> mySModelNodes = new HashMap<>();
  ...
  public void addDependencyForCurrent(SNode node) {
    ...
    if (node != null) {
      Set<SNode> hashSet = new HashSet<>(1);
      hashSet.add(node);
  ...
  public Set<Pair<SNode, List<IErrorReporter>>> getNodesWithErrors(boolean typesystemErrors) {
    if (typesystemErrors) {
      return new HashSet<>(super.getNodesWithErrors(typesystemErrors));
    }

    Set<Pair<SNode, List<IErrorReporter>>> result = new HashSet<>();
    Map<SNode, List<IErrorReporter>> nodesToErrorsMapNT = myNonTypeSystemComponent.getNodesToErrorsMap();
    Set<SNode> keySet = new HashSet<>(nodesToErrorsMapNT.keySet());
  ...
  private class MyModelListenerManager {
    private final Map<SModel, Integer> myNodesCount = new HashMap<>();
    ...
    void track(SNode node) {
      ...
      if (!myNodesCount.containsKey(sm)) {
        ...
        mySModelNodes.put(sm, new HashSet<>());
      } else {
    ...
    void updateGCedNodes() {
      // no-op: java.lang.ref.WeakReference/ReferenceQueue are not available in the J2CL
      // JRE emulation, so GC of tracked nodes cannot be detected here. Model listeners
      // are cleaned up via dispose() instead.
    }
