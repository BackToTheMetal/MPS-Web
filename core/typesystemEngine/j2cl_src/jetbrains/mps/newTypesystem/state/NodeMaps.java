...
/**
 * J2CL variant: {@code gnu.trove.THashMap} is not available in the J2CL build
 * (no J2CL-compatible Trove target), so this variant uses plain {@code HashMap}.
 */
public class NodeMaps {

  @StateObject
  private final Map<SNode, SNode> myNodesToTypes = new HashMap<>();
  @StateObject
  private final Map<SNode, SNode> myTypesToNodes = new HashMap<>();
  @StateObject
  private final Map<SNode, List<IErrorReporter>> myNodesToErrors = new HashMap<>();
