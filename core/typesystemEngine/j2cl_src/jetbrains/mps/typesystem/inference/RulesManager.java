...
/**
 * J2CL variant: {@code gnu.trove.THashSet}/{@code TObjectIntHashMap} are not available in
 * the J2CL build (no J2CL-compatible Trove target). This variant uses a plain {@code HashSet}
 * and takes the already-adapted {@link NamespaceRank} function directly (rather than a Trove
 * map), matching {@link AbstractLanguageProcessor}'s j2cl variant.
 */
public class RulesManager extends AbstractLanguageProcessor {
  ...
  private final Set<IVariableConverter_Runtime> myVariableConverters = new HashSet<>();
  ...
  @Override
  protected boolean processLoadedLangugage(LanguageRuntime language, NamespaceRank languageRanks) {
    ...
    try {
      myInferenceRules.addRuleSetItem(typesystem.getInferenceRules(), languageRanks);
      ...
      myComparisonRules.addRuleSetItem(comparisonRule_runtimes, languageRanks);
      myReplacementRules.addRuleSetItem(typesystem.getEliminationRules(), languageRanks);
      myVariableConverters.addAll(typesystem.getVariableConverters());
      myNonTypeSystemRules.addRuleSetItem(typesystem.getNonTypesystemRules(), languageRanks);
