 ...
 * @author Fedor Isakov
 *
 * J2CL variant: {@code gnu.trove.TObjectIntHashMap} is not available in the J2CL build
 * (no J2CL-compatible Trove target), so this variant uses a plain {@code HashMap<String, Integer>}.
 */
public abstract class AbstractLanguageProcessor {
  ...
  protected abstract boolean processLoadedLangugage(LanguageRuntime language, NamespaceRank languageRanks);
  ...
  protected synchronized void ensureUpToDate() {
    ...
    Map<String, Integer> languageRanks = new HashMap<>(sortedLanguages.size());
    int rank = 0;
    for (LanguageRuntime lang : sortedLanguages) {
      languageRanks.put(lang.getNamespace(), rank++);
    }
    NamespaceRank namespaceRank = (namespace) -> {
      Integer value = languageRanks.get(namespace);
    ...
    while (!myLanguagesToLoad.isEmpty()) {
      ...
      assert !myLoadedLanguages.contains(language);
      if (processLoadedLangugage(language, namespaceRank)) {
