 ...
 *
 * J2CL variant: {@code Class.isAssignableFrom} is not available in the J2CL JRE emulation
 * (no runtime reflection metadata), so {@link #selectProvider(Class)} matches providers by
 * exact query class equality instead of assignability.
 *
 * @author Fedor Isakov
 */
public class TypecheckingBackend implements CoreComponent {
  ...
  protected <Q extends TypecheckingQueries> TypecheckingProvider<Q> selectProvider(@NotNull Class<? extends Q> queryClass) {
    for (TypecheckingProvider<? extends TypecheckingQueries> provider : myProviders.values()) {
      if (queryClass.equals(provider.getQueriesClass())) {
