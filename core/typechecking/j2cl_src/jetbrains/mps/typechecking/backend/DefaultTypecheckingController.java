 ...
 * </blockquote>
 *
 * J2CL variant: {@code java.lang.ref.SoftReference} is not available in the J2CL JRE
 * emulation, so the on-demand session is held with a plain (strong) reference instead.
 * 
 * @author Fedor Isakov
 */
public class DefaultTypecheckingController extends TypecheckingController implements ParametersDiscoverable {
  ...
  // on demand session
  private TypecheckingSessionImpl myOnDemandSession;
  ...
  protected TypecheckingQueries getQueries(@NotNull SNode src, SNode trg, SConcept trgConcept, Flags flags) {
    ...
    TypecheckingSessionImpl runningSession = myOnDemandSession;
    if (runningSession == null || (runningSession.flags().getParamsMap() != flags.getParamsMap())) {
      ...
      this.myOnDemandSession = runningSession;
    }
  ...
  protected AuxDataContainer getDataContainer(TypecheckingProvider<?> provider) {
    ...
    if (runningSession == null) {
      runningSession = myOnDemandSession;
    }
