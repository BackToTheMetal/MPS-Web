 ...
 * Date: 8/27/12
 *
 * J2CL variant: {@code jetbrains.mps.util.SimpleLRUCache} is excluded from the J2CL build
 * (relies on unsupported concurrent collections), so this variant uses a plain, unbounded
 * {@code HashMap}-based cache instead. The J2CL/browser runtime is single-threaded, so no
...
public class LanguageScopeFactory implements CoreComponent, LanguageRegistryListener {
  ...
  private Map<LanguagesHolder, LanguagesHolder> myCachedLanguages;
  ...
  private LanguagesHolder getHolder(Collection<SLanguage> langs) {
    LanguagesHolder key = new LanguagesHolder(new ArrayList<>(langs));
    LanguagesHolder cached = myCachedLanguages.get(key);
    if (cached != null) {
      return cached;
    }
    myCachedLanguages.put(key, key);
    return key;
  }
