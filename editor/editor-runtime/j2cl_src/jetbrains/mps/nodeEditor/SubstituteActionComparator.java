...
public class SubstituteActionComparator implements Comparator<SubstituteAction> {
  ...
  protected int getRate(SubstituteAction action) {
    ...
    return matchingDegree(matchingText);
  }
  ...
  private int matchingDegree(String matchingText) {
    if (myPattern == null || myPattern.isEmpty()) {
      return 0;
    }
    String lowerText = matchingText.toLowerCase();
    String lowerPattern = myPattern.toLowerCase();
    if (lowerText.equals(lowerPattern)) {
      return Integer.MAX_VALUE;
    }
    if (lowerText.startsWith(lowerPattern)) {
      return 1000 - lowerText.length();
    }
    if (lowerText.contains(lowerPattern)) {
      return 500 - lowerText.length();
    }
