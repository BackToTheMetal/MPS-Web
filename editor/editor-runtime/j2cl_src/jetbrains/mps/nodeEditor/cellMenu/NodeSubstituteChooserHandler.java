...
public class NodeSubstituteChooserHandler {
  ...
  static final MatcherFactory CASE_INSENSITIVE_MATCHER_FACTORY = pattern -> new SimpleMatcher("*" + pattern);
  ...
  private NodeSubstituteInfoFilterDecorator createSubstituteInfoPatternMatchingDecorator(SubstituteInfo substituteInfo) {
    return new NodeSubstituteInfoFilterDecorator(substituteInfo, myEditorCell.getContext().getRepository()) {
      ...
      protected Predicate<SubstituteAction> createFilter(String pattern) {
        SimpleMatcher matcher = CASE_INSENSITIVE_MATCHER_FACTORY.createMatcher(pattern);
        return action -> {
          if (pattern == null) {
            return true;
          }
  ...
  interface MatcherFactory {
    SimpleMatcher createMatcher(String pattern);
  }
  ...
  static class SimpleMatcher {
    ...
    boolean isStartMatch(String text) {
      if (text == null) return false;
      String lower = text.toLowerCase();
      String p = myPattern;
      if (p.startsWith("*")) p = p.substring(1);
    ...
    Iterable<int[]> matchingFragments(String text) {
      ...
      return java.util.Collections.singletonList(new int[]{idx, idx + p.length()});
    }
  }
}
