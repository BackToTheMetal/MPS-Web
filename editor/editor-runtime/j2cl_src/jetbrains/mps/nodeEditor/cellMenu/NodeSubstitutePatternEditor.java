...
public class NodeSubstitutePatternEditor {
  ...
  private static boolean isReallyTypedEvent(KeyEvent e) {
    char c = e.getKeyChar();
    return e.getID() == KeyEvent.KEY_TYPED && c != KeyEvent.CHAR_UNDEFINED && !e.isControlDown() && !e.isAltDown() && !e.isMetaDown();
  }
  ...
  private class TextLineDelegate implements TextLineOperations {
    ...
    @Override
    public boolean processKeyTyped(KeyEvent keyEvent) {
      if (isReallyTypedEvent(keyEvent)) {
  ...
  private static class EditorWindow extends JWindow implements TextLineOperations {
    ...
    EditorWindow(Window owner, EditorContext context) {
      ...
      if (context != null) {
        ...
        setFont(((EditorComponentSettingsImpl) context.getEditorComponent().getEditorComponentSettings()).getDefaultFont());
