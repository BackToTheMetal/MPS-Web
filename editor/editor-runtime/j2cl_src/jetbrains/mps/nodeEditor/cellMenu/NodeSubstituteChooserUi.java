...
class NodeSubstituteChooserUi implements ISubstituteChooserUi {
  ...
  public void refreshUi(boolean recalculateSize) {
    if (recalculateSize) {
      resetListSize();
      myPopupContent.validate();
      myPopupContent.setSize(myPopupContent.getPreferredSize());
      resetRelativePosition(myPopupContent.getPreferredSize());
      updateLocation();
    }
  ...
  public void show() {
    ...
    myScrollPane = new JScrollPane(myList, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
    ...
    myPopupContent = mainPanel;
    myPopup = new JPopupMenu();
    myPopup.setLayout(new BorderLayout());
    myPopup.add(mainPanel, BorderLayout.CENTER);
    myPopup.setMinimumSize(new Dimension(MY_MIN_CELL_WIDTH, 0));

    Dimension preferredSize = myPopupContent.getPreferredSize();
    initRelativePosition(preferredSize);
    Point location = calculateLocation(preferredSize);
    myPopup.setLocation(location);
    myPopup.show(myNodeSubstituteChooser.getEditorWindow(), location.x, location.y);
    myList.ensureIndexIsVisible(myList.getSelectedIndex());
  ...
  public void updateLocation() {
    Point newLocation = calculateLocation(myPopupContent.getPreferredSize());
